...
# 3. 데이터 로드 (smart_read) - [지능형 경로 적용]
@st.cache_data
def load_data():
    # 파이썬 파일이 있는 현재 위치를 자동으로 파악합니다.
    current_dir = os.path.dirname(os.path.abspath(__file__))
    
    def smart_read(file_name):
        # 파일이 파이썬 파일 바로 옆에 있는지 확인하여 경로를 생성합니다.
...
# 4. 화면 구성 (원본 로직 그대로 유지)
if store_df is None:
    st.error("파일을 찾을 수 없습니다. 파이썬 파일과 같은 폴더에 'store_data.csv'와 'age.csv'가 있는지 확인해 주세요.")
    st.stop()
